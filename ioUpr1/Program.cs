	...
	internal class Program
	{
		...
		static void northWestCornerMethod(int[] supply, int[] demand, int[,] matrix, /*bool[,] marked,*/ int row, int col, List<int[]> list1, List<int[]> list2)
		{
			...
			while (row < matrix.GetLength(0) && col < matrix.GetLength(1))
			{
				...
				//update s/d values and num of empty cols
				//⚠only iterate either or so that base 0s are kept
				supply[row] -= lower;
				demand[col] -= lower;
		...
		static void minimalElement(int[] supply, int[] demand, int[,] matrix, int[,] cmatrix, List<int[]> list1, List<int[]> list2)
		{
			int emptycol = 0;
			//init empty
			for (int i = 0; i < matrix.GetLength(0); i++)
			...
			while (supply.Sum() > 0 && demand.Sum() > 0)
			{
				...
				foreach (var e in list2)
				{
					...
					if (demand[j] == 0) {
						//only skip if zeroed cols are as much as empty ones in order to keep base 0s
		...
		static void Main(string[] args)
		{
			...
			var matrix = new int[supply.Length, demand.Length];
			...
			//northWestCornerMethod(supply, demand, matrix, 0, 0, full, empty);
			minimalElement(supply, demand, matrix, cmatrix, full, empty);
			Console.WriteLine("X0");
			PrintMatrix(matrix);
			
			distributionMethod(matrix, full, empty, cmatrix, 5);
			//potentialMethod(matrix, full, empty, cmatrix, 5);
            Console.WriteLine("Xopt");
            PrintMatrix(matrix);
            //for (int i = 0; i < matrix.GetLength(0); i++)
            //{
            //	for (int j = 0; j < matrix.GetLength(1); j++)
